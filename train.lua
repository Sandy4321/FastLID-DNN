...
if opt.network == '' then
    ...
    outputs = 13        -- number of classes (all 12 languages + OOS just in case)
...
local validate_file="/pool001/atitus/FastLID-DNN/data_prep/feats_all/" .. opt.languages .. "_validate"

-- Balance the data
--local label2maxframes = torch.zeros(4)
local label2maxframes = torch.zeros(13)
local min_frames = 19251        -- Count for Vietnamese, the minimum of all languages in set
...
local train_file="/pool001/atitus/FastLID-DNN/data_prep/feats_all/" .. opt.languages .. "_train"

-- Balance the data
--local label2maxframes = torch.zeros(4)
local label2maxframes = torch.zeros(13)
local min_frames = 207000       -- Count to keep memory footprint low for 13 languages
...
for epoch = 1,opt.epochs do
    ...
    local max_utterances = 765   -- Total in validation set for all languages
